...
dict = array(PHI_pre)
absdict_pre = [[abs(x) for x in y] for y in PHI_pre]
absdict = array(absdict_pre)
numNodes = len(dict)
...
input=net.make_input('input',clippre[0])
...
net.connect(neuron,salience,func=absn,pstc=.005)
net.connect(salience,salience,transform=salrec,func=rectn,pstc=.001)
net.connect(salience,sal_decode,transform=absdict.T,func=rectn,pstc=.001)
